...
{
    fGeantpyDir = new G4UIdirectory("/geantpy/");
    fGeantpyDir->SetGuidance("UI commands specific to Geantpy setup.");

    fDetDir = new G4UIdirectory("/geantpy/det/");
    fDetDir->SetGuidance("Detector construction control.");

    fWorldMatCmd = new G4UIcmdWithAString("/geantpy/det/setWorldMaterial", this);
    fWorldMatCmd->SetGuidance("Select material of the world.");

    fChamberMatCmd = new G4UIcmdWithAString("/geantpy/det/setChamberMaterial", this);
    fChamberMatCmd->SetGuidance("Select material of the chamber.");

    fChamberWidthCmd = new G4UIcmdWithADoubleAndUnit("/geantpy/det/setChamberWidth", this);
    fChamberWidthCmd->SetGuidance("Set the width/height of the chamber.");
    fChamberWidthCmd->SetUnitCategory("Length");

    fChamberLengthCmd = new G4UIcmdWithADoubleAndUnit("/geantpy/det/setChamberLength", this);
    fChamberLengthCmd->SetGuidance("Set the length (Z) of the chamber.");
    fChamberLengthCmd->SetUnitCategory("Length");

    fTargetMatCmd = new G4UIcmdWithAString("/geantpy/det/setTargetMaterial", this);
    fTargetMatCmd->SetGuidance("Select material of the target.");

    fTargetShapeCmd = new G4UIcmdWithAString("/geantpy/det/setTargetShape", this);
