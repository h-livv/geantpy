...
EXECUTABLE = ENGINE_DIR / "build" / "geantpy"
...
class Simulation:
    ...
    def __init__(self):
        ...
        self.output_filter = "All" #"All" or "Antimatter"
    ...
    def generate_macro(self, interactive=False):
        ...
        lines.extend([
            f"/geantpy/det/setWorldMaterial {self.environment.world_material}",
            f"/geantpy/det/setChamberMaterial {self.environment.chamber_material}",
            f"/geantpy/det/setChamberWidth {self.environment.chamber_width}",
            f"/geantpy/det/setChamberLength {self.environment.chamber_length}",
            f"/geantpy/det/setTargetShape {self.environment.target_shape}",
            f"/geantpy/det/setTargetMaterial {self.environment.target_material}",
            f"/geantpy/det/setTargetWidth {self.environment.target_width}",
            f"/geantpy/det/setTargetLength {self.environment.target_length}",
            f"/geantpy/det/setTargetPosition {self.environment.target_position}",
            f"/geantpy/output/setFilter 1" if self.output_filter.lower() == "antimatter" else f"/geantpy/output/setFilter 0",
            f"/geantpy/output/setLightFilter 1" if self.drop_light_particles else f"/geantpy/output/setLightFilter 0",
        ])
        ...
        if self.tracking_cut is not None:
            lines.append(f"/geantpy/tracking/setEnergyCut {self.tracking_cut}")
        
        lines.append(f"/geantpy/tracking/saveSecondaries {'true' if self.save_secondaries else 'false'}")
        lines.append(f"/geantpy/tracking/recordMode {self.record_mode}")
        ...
        if interactive:
            lines.extend([
                ...
                "/vis/modeling/trajectories/drawByParticleID-0/set proton orange",      # Primary Beam
                ...
                "/vis/scene/add/text2D 0.9 -.9 24 ! ! GeantPy",
                "/vis/set/textLayout",
                "/vis/set/textColour",
                ...
                "/vis/geometry/set/colour Chamber 1 1 1 1 0.01",
    ...
    def run(self, interactive=False):
        ...
        if HARDCODED_H5_VAL.exists() and HARDCODED_H5_SIM.exists():
            ...
            print(f"[+] Run packaged successfully in: data/runs/{run_name}/\n")
        else:
